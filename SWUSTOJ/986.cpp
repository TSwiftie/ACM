...
void ccode(haffnode hafftree[], int n)
{
	char sh[maxn];
	scanf("%s",sh);
	int root = hafftree[1].parent;
	root = hafftree[root].parent;
	for(int i = 0;i < strlen(sh);i++)
	{
		int temp = root;
		while(hafftree[temp].leftchild!=-1&&hafftree[temp].rightchild!=-1)
		{
			if(sh[i]=='0')
			{
				temp = hafftree[temp].leftchild;
			}
			else
			{
				temp = hafftree[temp].rightchild;
			}
			i++;
		}
		i--;
		printf("%c",hafftree[temp].ch);
	}
}
int main(void)
{
